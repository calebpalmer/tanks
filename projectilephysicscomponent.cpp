...
ProjectilePhysicsComponent::ProjectilePhysicsComponent(
    int in_width, int in_height, CapEngine::Vector in_velocity)
...
{
    return std::make_unique<ProjectilePhysicsComponent>(m_width, m_height,
                                                        m_velocity);
}

//! \copydoc Component::update
void ProjectilePhysicsComponent::update(CapEngine::GameObject &in_object,
                                        double ms)
{
    ...
    in_object.setPosition(CapEngine::applyDisplacement(
        in_object.getVelocity(), in_object.getPosition(), ms));
}
...
/**
 ...
 \return
   The component.
*/
std::unique_ptr<ProjectilePhysicsComponent>
    ProjectilePhysicsComponent::makeComponent(const jsoncons::json &in_json)
{
    try {
        ...
        // get the width
        CapEngine::ComponentUtils::assertPropertyExists(
            in_json, ProjectilePhysicsComponent::kWidthPropertyName);
        const auto width =
            in_json[ProjectilePhysicsComponent::kWidthPropertyName].as_int();
        // get the height
        CapEngine::ComponentUtils::assertPropertyExists(
            in_json, ProjectilePhysicsComponent::kHeightPropertyName);
        const auto height =
            in_json[ProjectilePhysicsComponent::kHeightPropertyName].as_int();

        // get the velocity
        CapEngine::ComponentUtils::assertPropertyExists(
            in_json, ProjectilePhysicsComponent::kVelocityPropertyName);
        const auto velocity = CapEngine::JSONUtils::readVector(
            in_json[ProjectilePhysicsComponent::kVelocityPropertyName]);

        return std::make_unique<ProjectilePhysicsComponent>(width, height,
                                                            velocity);
    }
...
/**
 \param in_factory
   The factory.
*/
void ProjectilePhysicsComponent::registerConstructor(
...
std::optional<CapEngine::Rectangle> ProjectilePhysicsComponent::boundingPolygon(
    const CapEngine::GameObject &in_object) const
{
    const auto position = in_object.getPosition();
    const auto rectangle = CapEngine::Rectangle{
        static_cast<int>(position.getX() - (m_width / 2.0)),
        static_cast<int>(position.getY() - (m_height / 2)), m_width, m_height};

    return rectangle;
}

bool ProjectilePhysicsComponent::handleCollision(
