...
class TankPhysicsComponent final : public CapEngine::PhysicsComponent {
  public:
    TankPhysicsComponent(std::optional<CapEngine::Vector> in_projectileOffset = std::nullopt);
    ~TankPhysicsComponent() override = default;

    std::unique_ptr<Component> clone() const override;

    static std::unique_ptr<TankPhysicsComponent> makeComponent(const jsoncons::json &in_json);
    ...
    void receive(CapEngine::GameObject &in_object, int in_messageId, const std::string &in_message) override;
